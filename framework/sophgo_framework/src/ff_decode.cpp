...
decoder::decoder(int devId) {
    auto ret = bm_dev_request(&m_handle, devId);
    m_end = false;
    m_videoDecoder = new VideoDecFFM();
}
...
bm_image decoder::decodeJpg(std::string jpg_path){
    bm_image bmimg;
    picDec(m_handle, jpg_path.c_str(), bmimg);
    return bmimg;
}

int decoder::decodeVideo(std::string video_path) {
    m_videoDecoder->openDec(&m_handle, video_path.c_str());
    return 0;
}

int decoder::decodeVideoGrab(bm_image& img){
    if (m_videoDecoder == nullptr) {
        std::cout << "video decoder is null." << std::endl;
        return -1;
    }

    if (m_end) {
        std::cout << "video decoder meet end." << std::endl;
        return -1;
    }

    bm_image* img_ = m_videoDecoder->grab();
    if (!img_){
        m_end = true;
        std::cout << "maybe meet endof."<<std::endl;
        return -1;
    }else {
        img = *img_;
        return 0;
